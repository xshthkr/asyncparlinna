 ...
 *      Author: xshthkr
 *
 * Adapted from kokofan's implementation of the scattered algorithm for MPI_Alltoallv, with CUDA support for GPU buffers.
*/
	...
	MPI_Comm comm)
{
	...
	for (ii = 0; ii < comm_size; ii += bblock) {
		...
		for (i = 0; i < ss; i++) {
			...
			if (recvcounts[dst]) {
				mpi_errno = MPI_Irecv((char *) d_recvbuf + rdispls[dst] * rext,
		...
		for (i = 0; i < ss; i++) {
			...
			if (sendcounts[dst]) {
				mpi_errno = MPI_Isend((char *) d_sendbuf + sdispls[dst] * sext,
